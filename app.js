...
function load(){ try{ const raw=localStorage.getItem('bb_data'); if(raw){ const data=JSON.parse(raw); Object.assign(state, data); if(!state.ui) state.ui={}; if(typeof state.ui.calOffset==='undefined') state.ui.calOffset=0; if(!state.ui.spMode) state.ui.spMode='list'; } }catch(e){ console.error(e); } }
...
function planForPaycheck(p){ const bills=billsBefore(p.date); const d=new Date(p.date); const m=d.getMonth(), y=d.getFullYear();
  const numChecksThisMonth=state.paychecks.filter(q=>{ const dq=new Date(q.date); return dq.getMonth()===m && dq.getFullYear()===y; }).length || 1; const mt=monthlyTotals(d);
...
function renderCalendar(){ const base=monthFromOffset(state.ui.calOffset); const year=base.getFullYear(), month=base.getMonth();
  ...
  const paydays=getPaydaysFor(month+1, year).map(d=>d.getDate()); let sumBills=0, sumSubs=0, sumOther=0;
  for(let day=1; day<=daysInMonth; day++){ const cell=document.createElement('div'); cell.className='cell'; const inner=document.createElement('div'); inner.className='inner';
    ...
    let dayTotal=0;
    state.expenses.filter(e=> Number(e.due)===day).forEach(e=>{ const b=document.createElement('div'); b.className='badge bill'; b.textContent=e.name; inner.appendChild(b); dayTotal+=Number(e.amount||0);
      const name=(e.name||'').toLowerCase(); if(/netflix|hulu|prime|spotify|apple|flex|xbox|playstation/.test(name)) sumSubs+=Number(e.amount||0);
      else if(/water|power|electric|rent|mortgage|wifi|internet|gas|sewer|trash/.test(name)) sumBills+=Number(e.amount||0);
      else sumOther+=Number(e.amount||0);
    });
  ...
  const s=$('#calSummary'); s.innerHTML=`<div class="pill">Bills & Utilities ${currency(sumBills)}</div><div class="pill">Subscriptions ${currency(sumSubs)}</div><div class="pill">Other ${currency(sumOther)}</div>`;
}
...
function drawPieChart(svg,data){ const tot=data.reduce((s,d)=>s+d[1],0)||1; const cx=80,cy=80,r=70; let a0=0; svg.innerHTML=""; data.forEach(([k,v],i)=>{ const a1=a0+(v/tot)*Math.PI*2; const large=(a1-a0)>Math.PI?1:0; const x0=cx+r*Math.cos(a0), y0=cy+r*Math.sin(a0); const x1=cx+r*Math.cos(a1), y1=cy+r*Math.sin(a1); const path=document.createElementNS('http://www.w3.org/2000/svg','path'); const d=`M ${cx} ${cy} L ${x0} ${y0} A ${r} ${r} 0 ${large} 1 ${x1} ${y1} Z`; path.setAttribute('d',d); path.setAttribute('fill','var(--primary)'); path.setAttribute('opacity',0.7+0.3*(i%2)); svg.appendChild(path); a0=a1; }); }
...
function setupEvents(){
  ...
  $('#settingsBtn')?.addEventListener('click', ()=>{
    ...
    $('#setPaydays').value = (state.settings.paydays||[]).join(',');
  ...
  dlg?.addEventListener('input', (e)=>{
    ...
    if(e.target.id==='setPaydays'){ state.settings.paydays = String(e.target.value||'15,30').split(',').map(s=>parseInt(s,10)).filter(n=>!isNaN(n)); }
  ...
  $('#restoreBtn')?.addEventListener('click', async ()=>{ const file=$('#restoreFile').files[0]; if(!file) return alert('Choose a backup file first.');
    try{ const data=JSON.parse(await file.text()); Object.assign(state, {...state, ...data}); if(!state.ui) state.ui={}; if(typeof state.ui.calOffset==='undefined') state.ui.calOffset=0; save(); renderDashboard(); renderCalendar(); renderDebts(); renderSpending(); renderEnvelopes(); alert('Restore complete.'); }catch(e){ alert('Invalid file.'); } });
