...
function centersAndWidths(){
  ...
  return tabs.map(t=>{const r=t.getBoundingClientRect();return {center:(r.left-br.left)+r.width/2,width:r.width};});
}
function setPadAtIndex(i){
  const arr=centersAndWidths(); const a=arr[i]; if(!a) return;
  const pad=$('#activePad'); const x=Math.round(a.center - a.width/2) + 12; // align with left inset
  pad.style.width=Math.round(a.width-8)+'px'; pad.style.transform=`translateX(${x}px)`;
}
function setPadBetween(i, frac){
  ...
  const pad=$('#activePad'); const x=Math.round(center - width/2) + 12;
  pad.style.width=Math.round(width-8)+'px'; pad.style.transform=`translateX(${x}px)`;
}
...
/* Swipe (axis-locked) with pad interpolation */
(()=>{
  ...
  function move(e){
    ...
    if(axis==='x'){ e.preventDefault(); const base=-100*cur; const pct=dx/window.innerWidth; tr.style.transform=`translate3d(calc(${base}vw + ${pct*100}vw),0,0)`; const frac=Math.min(1,Math.max(-1,pct)); if((cur>0&&pct>0)||(cur<IDS.length-1&&pct<0)){ setPadBetween(cur, Math.sign(pct)*Math.abs(frac)); } }
