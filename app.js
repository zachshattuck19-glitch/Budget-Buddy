...
function showPanel(id){ $$('.panel').forEach(p=> p.classList.toggle('active', p.id===id));
  $$('.t').forEach(b=> b.classList.toggle('active', b.dataset.tab===id)); localStorage.setItem('bb_tab', id);
  if(id==='spending') renderSpending(); if(id==='calendar') renderCalendar(); if(id==='debts') renderDebts(); if(id==='budget') renderEnvelopes();
}
function initNav(){ document.querySelector('.tabbar').addEventListener('click', (e)=>{ const b=e.target.closest('.t'); if(!b) return; showPanel(b.dataset.tab); });
...
function billsBefore(dateISO){ const d=new Date(dateISO); const paydays=getPaydaysFor(d.getMonth()+1, d.getFullYear()); let next=null; for(const p of paydays){ if(p>d){ next=p; break; } }
  ...
  const lastDay=new Date(next.getFullYear(), next.getMonth(), 0).getDate(); const dueLimit=next.getDate();
...
function monthlyTotals(){ const now=new Date(); const m=now.getMonth(), y=now.getFullYear();
  ...
  const wig=inc*(Number(state.settings.wiggle)/100); const spend=inc*(Number(state.settings.spend)/100);
  const leftover=inc-(exp+mins+wig+spend); return {inc,exp,mins,wig,spend,leftover};
}
function planForPaycheck(p){ const bills=billsBefore(p.date); const d=new Date(p.date); const m=d.getMonth(), y=d.getFullYear();
  ...
  const mt=monthlyTotals(); const wigPer=(mt.wig/Math.max(1,numChecksThisMonth)); const minsPer=(mt.mins/Math.max(1,numChecksThisMonth));
  const spendable = Number(p.amount) - (bills + wigPer + minsPer); const extraDebt=Math.max(0, spendable); const shortfall=Math.max(0, -(spendable));
...
function renderDashboard(){ const now=new Date(); const showPd=getPaydaysFor(now.getMonth()+1, now.getFullYear()).some(d=> d.getDate()===now.getDate()); $('#paydayBanner').hidden=!showPd;
  $('#pcDate').value=todayISO(); const qp=$('#quickPaydays'); qp.innerHTML="";
  getPaydaysFor(now.getMonth()+1, now.getFullYear()).forEach(d=>{ const b=document.createElement('button'); b.type='button'; b.textContent=`Use ${d.toLocaleDateString()}`;
  ...
  const latest=state.paychecks.slice().sort((a,b)=> new Date(b.date)-new Date(a.date))[0] || {date: todayISO(), amount: 0, kind:'regular'};
  const plan=planForPaycheck(latest); $('#spendable').textContent=currency(plan.spendable);
  const det=$('#pcDetails'); det.innerHTML=""; [['Bills before next payday', plan.bills], ['Cushion (wiggle share)', plan.wigPer], ['Debt minimum share', plan.minsPer], ['Extra debt (if any)', plan.extraDebt]]
    .forEach(([k,v])=>{ const row=document.createElement('div'); row.className='row'; row.innerHTML=`<div>${k}</div><div>${currency(v)}</div>`; det.appendChild(row); });
...
// ---- Calendar
function renderCalendar(){ const grid=$('#calendarGrid'); grid.innerHTML=""; const now=new Date(); const year=now.getFullYear(), month=now.getMonth();
  ...
  for(let day=1; day<=daysInMonth; day++){ const cell=document.createElement('div'); cell.className='day'; cell.innerHTML=`<div class="dnum">${day}</div>`;
...
function renderSpending(){ $$('#spView .seg').forEach(s=> s.classList.toggle('active', s.dataset.mode===state.ui.spMode));
  const data=spendingByCategory(); const lst=$('#spList'); const svgb=$('#spBar'); const svgp=$('#spPie'); lst.innerHTML=""; svgb.setAttribute('hidden', true); svgp.setAttribute('hidden', true); lst.hidden=false;
...
function drawPieChart(svg, data){ const tot=data.reduce((s,d)=>s+d[1],0)||1; const cx=80,cy=80,r=70; let a0=0; svg.innerHTML="";
  data.forEach(([k,v],i)=>{ const a1=a0+(v/tot)*Math.PI*2; const large=(a1-a0)>Math.PI?1:0; const x0=cx+r*Math.cos(a0), y0=cy+r*Math.sin(a0); const x1=cx+r*Math.cos(a1), y1=cy+r*Math.sin(a1);
...
// ---- Envelopes
function renderEnvelopes(){ const tbody=$('#envTable tbody'); if(!tbody) return; tbody.innerHTML="";
  ...
  cats.forEach(cat=>{ const it=state.envelopes.items[cat]||(state.envelopes.items[cat]={alloc:0,rollover:0,sinking:false,goal:0});
    const tr=document.createElement('tr'); tr.innerHTML=`<td>${cat}</td>
...
function setupEvents(){ $('#settingsBtn')?.addEventListener('click', ()=> alert('Settings drawer coming soon (wiggle %, spend %, paydays, theme).'));
  ...
  $('#restoreBtn')?.addEventListener('click', async ()=>{ const file=$('#restoreFile').files[0]; if(!file) return alert('Choose a backup file first.'); try{ const data=JSON.parse(await file.text()); Object.assign(state, { **state, ...data }); save(); renderDashboard(); renderCalendar(); renderDebts(); renderSpending(); renderEnvelopes(); alert('Restore complete.'); }catch(e){ alert('Invalid file.'); } });
...
// ---- Init + Splash safety fallback
if('serviceWorker' in navigator){ window.addEventListener('load', ()=> navigator.serviceWorker.register('sw.js')); }
load(); applyTheme(state.settings.theme||'system');
document.addEventListener('DOMContentLoaded', ()=>{ renderDashboard(); renderCalendar(); renderDebts(); renderSpending(); renderEnvelopes(); initNav(); setupEvents();
  // If CSS splash didn't run for any reason, force-hide after 1.2s
